...
cx_Aeg_Lua = {
	debug = {
		...
		-- 普通输出(带换行、不带换行)
		,println = function (msg,level)
			if level == nil then level = cx_Aeg_Lua.debug.def_level end
			if level > 5 then return end
			if msg ~= nil then 
				aegisub.debug.out(level, tostring(msg).."\n") 
			else
				aegisub.debug.out(level, "\n")
			end
		end
		...
		,var_dump = function (value,level)
			...
			local function print(msg,level)
				cx_Aeg_Lua.debug.println(msg,level)
			end
			-- 好用的table输出函数
			local function print_r(t,level)  
				local print_r_cache={}
				local function sub_print_r(t,indent,level)
					if (print_r_cache[tostring(t)]) then
						print(indent.."*"..tostring(t),level)
					else
						print_r_cache[tostring(t)]=true
						if (type(t)=="table") then
							for pos,val in pairs(t) do
								if (type(val)=="table") then
									print(indent.."["..pos.."] => "..tostring(t).." {",level)
									sub_print_r(val,indent..string.rep(" ",string.len(pos)+8),level)
									print(indent..string.rep(" ",string.len(pos)+6).."}",level)
								elseif (type(val)=="string") then
									print(indent.."["..pos..'] => "'..val..'"',level)
								else
									print(indent.."["..pos.."] => "..tostring(val),level)
								end
							end
						else
							print(indent..tostring(t),level)
	...
	,display = {
		...
		,select_file_last_select_path = nil
...
if config.out_level ~= nil then cx_Aeg_Lua.debug.def_level = config.out_level end
...
function fx_help.Ck_edit:display_edit(subs,def_text,start,edit_num)
	...
	dialog={
		...
		{class="textbox",name="text", value = def_text, width=50,height=20, x=0, y=1},
	}
	buttons=nil
	button_ids=nil
	button, result_table = aegisub.dialog.display(dialog, buttons, button_ids)
...
function fx_help.vc:create(subs, selected_lines, active_line)
	...
	pre_text = '{\\1vc('..res[1].c..','..res[2].c..','..res[3].c..','..res[4].c..')'
	pre_text = pre_text..'\\1va('..res[1].a..','..res[2].a..','..res[3].a..','..res[4].a..')}'
